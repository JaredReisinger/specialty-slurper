...
const dayEventSelector = select.compile(
  'div.calendar-list-item__info-title > a',
);
...
const eventSpecialtySelector = select.compile(
  'div.item-detail__event-misc-title',
);

const breedTableSelector = select.compile('table');
const breedSelector = select.compile('td[colspan="4"] strong a strong');

const hostSelector = select.compile(
  'td[colspan="8"] > div[align="center"] > font > font > b',
);
const dateSelector = select.compile(
  'td[colspan="8"] > div[align="center"] + font > center',
);
const locationSelector = select.compile(
  'td[colspan="8"] > div[align="center"] + font > center + font > font > center',
);

const judgeSelector = select.compile('td[colspan="4"] + td a.white');
const entriesSelector = select.compile(
  'td[colspan="4"] + td + td[colspan="2"] font',
);
...
function parseEvent(eventLink, body) {
  ...
  return parseHtml(body).then(function (dom) {
    // logger.inspect(dom);
    ...
    _.each(events, function (evt) {
      ...
      if (
        _.startsWith(eventType, 'Specialty') ||
        _.startsWith(eventType, 'Parent Specialty')
      ) {
        ...
        if (breed === 'Basenji') {
          ...
          if (breedTable) {
            ...
            _.each(breeds, function (cls) {
              ...
              var scriptLink = htmlparser.DomUtils.getAttributeValue(
                breedAnchor,
                'href',
              );
      ...
      switch (item.eventMeta) {
        ...
        case 'placement':
          ...
          var nameEl = findFollowing(item, function (node) {
            return node.type === 'tag' && node.name === 'td';
          });
