...
function createProgress(label, steps) {
  ...
  if (program.progress) {
    var barFormat = sprintf(
      '%8s [:bar] :percent complete, :etas remaining',
      label,
    );
...
function createRange() {
  ...
  var end = moment('2018-12-31');
  // var end = moment('2018-03-31');
...
function getAndParseBreed(link) {
  return client.makeAkcRequest(link).then(parser.parseBreed).tap(progressTick);
}
