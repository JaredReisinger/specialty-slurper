...
const client = require('./akc-client');
const parser = require('./akc-parser');
...
function createProgress(label, steps) {
    ...
    if (program.progress) {
        var barFormat = sprintf('%8s [:bar] :percent complete, :etas remaining', label);
...
function createRange() {
    ...
    // var start = moment('2017-06-16');
    // var start = moment('2017-08-12');
    var end = moment('2017-12-31');
    // var end = moment('2017-04-01');
    // var end = moment('2017-06-16');
    // var end = moment('2017-08-12');
...
function getAndParseDay(date) {
    return client.getDayEvents(date)
        .then(_.partial(parser.parseDayEvents, date))
        .tap(progressTick);
}

function getAndParseEvent(link) {
    return client.makeAkcRequest(link)
        .then(_.partial(parser.parseEvent, link))
        .tap(progressTick);
}
...
function requireNonZero(name, data) {
    if (data.length === 0) {
        throw new Error(sprintf('No data for %s.', name));
    }
...
function reportResults(eventsResults) {
    ...
    _.each(eventsResults, function(eventResults) {
        ...
        csvLine(sprintf('Judge: %s', eventResults.judge));
        ...
        _.each(eventResults.sections, function(section, sectionIndex) {
            ...
            if (sectionIndex === 0) {
                sectionName = sprintf('%s %s', sectionName, eventResults.entries);
            }
            ...
            _.each(section.placements, function(placement) {
                ...
                if (program.formatPublishing) {
                    csvLine('', '', '', placementName,
                        sprintf('%s, %s', placement.name.toUpperCase(), placement.owner));
                } else {
            ...
            _.each(section.classes, function(cls) {
                ...
                _.each(cls.placements, function(placement) {
                    ...
                    _.each(winnersMap[placement.name], function(winningAbbrev) {
                        placementName = sprintf('%s/%s', placementName, winningAbbrev);
                    });
                    if (program.formatPublishing) {
                        csvLine('', '', '', placementName,
                            sprintf('%s, %s', placement.name.toUpperCase(), placement.owner));
